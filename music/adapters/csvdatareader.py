...
# extract image_file from track_row
def extract_image_file(track_row: dict):
    image_file = track_row['track_image_file']
    if image_file == 'None':
        image_file = None
    # remove https://freemusicarchive.org/file/images/albums/
    else:
        image_file = image_file[48:]
    # add https://freemusicarchive.org/image/?file=images%252Falbums%252F to the start and &width=290&height=290&type=image to the end
    image_file = 'https://freemusicarchive.org/image/?file=images%2Falbums%2F' + image_file + '&width=290&height=290&type=image'
    return image_file

def extract_track_file(track_row: dict):
    track_file = track_row['track_file']
    if track_file == 'None':
        track_file = None
    # add https://files.freemusicarchive.org//storage-freemusicarchive-org//
    else:
        start = 'https://files.freemusicarchive.org//storage-freemusicarchive-org//'
        # for each '/' in track_file, add another '/'
        output2 = start + track_file.replace('/', '//')
    return output2
...
class TrackCSVReader:
    ...
    def __init__(self, albums_csv_file: str, tracks_csv_file: str):
        ...
        # Set of unique image files
        self.__dataset_of_image_files = set()
        # Set of unique track files
        self.__dataset_of_track_files = set()
    ...
    @property
    def dataset_of_image_files(self) -> set:
        return self.__dataset_of_image_files
    
    @property
    def dataset_of_track_files(self) -> set:
        return self.__dataset_of_track_files
    ...
    def read_csv_files(self):
        ...
        for track_row in track_rows:
            ...
            # Extract image_file attribute and assign image_file to the track.
            image_file = extract_image_file(track_row)
            track.image_file = image_file

            # Extract track_file attribute and assign track_file to the track.
            track_file = extract_track_file(track_row)
            ...
            # Populate dataset for image_file
            if image_file is not None and image_file not in self.__dataset_of_image_files:
                self.__dataset_of_image_files.add(image_file)
            
            # Populate dataset for track_file
            if track_file is not None and track_file not in self.__dataset_of_track_files:
                self.__dataset_of_track_files.add(track_file)
